...
export function FluidMorphBg({
  ...
  return (
    <div
      className={"fluid-morph-bg-wrapper relative overflow-hidden w-full h-full " + className}
      style={{ backgroundColor }}
      aria-hidden="true"
    >
      <svg
        className="fluid-morph-svg absolute inset-0 w-full h-full object-cover"
        preserveAspectRatio="none"
        viewBox={viewBox}
      >
        {paths.map((dList, index) => {
          const variance = (index % 3) * 0.8;
          const pathDuration = (duration + variance).toFixed(1) + "s";
          const delay = (index * 0.35).toFixed(2) + "s";
          const fillColor = colors[index % colors.length];

          return (
            <path key={index} d={dList[0]} fill={fillColor}>
              <animate
                attributeName="d"
                dur={pathDuration}
                begin={delay}
                repeatCount="indefinite"
                values={dList[0] + "; " + dList[1] + "; " + dList[0]}
                keyTimes="0; 0.5; 1"
                calcMode="spline"
      ...
      {overlay && <div className="fluid-morph-overlay" />}
    </div>
  );
}

export default FluidMorphBg;
